...
try:
    ...
    logger.info("✅ Đã kết nối Google Sheet")
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "main_menu":
        ...
        await log_user_to_sheet(user, MENU[index][0])
...
async def log_user_to_sheet(user, action):
    if sheet is None:
        logger.warning("❌ Sheet không khả dụng, không thể ghi log.")
        return
    ...
    try:
        ...
        logger.info(f"Ghi log: {user.username} - {action}")
    except Exception as e:
        logger.warning(f"Lỗi khi ghi log vào Google Sheets: {e}")
